...
class CartAdapter(public var cartItems: MutableList<String>, private val context: Context) :
    ...
    override fun onBindViewHolder(holder: CartViewHolder, position: Int) {
        ...
        // При клике на изображение открываем подробности товара
        holder.image.setOnClickListener {
            val intent = Intent(context, ItemActivity::class.java).apply {
                ...
                putExtra("itemPrice", item.price)  // Передаем цену товара
            }
        ...
        holder.deleteBtn.setOnClickListener {
            cartItems.removeAt(position)  // Удаляем товар из списка корзины
            notifyItemRemoved(position)  // Уведомляем адаптер о том, что товар удален
            notifyItemRangeChanged(position, cartItems.size)  // Обновляем оставшиеся элементы

            // Обновляем корзину в SharedPreferences
            val updatedSet = cartItems.toSet()
            sp.edit().putStringSet("cart", updatedSet).apply()

            // Удаляем информацию о товаре из SharedPreferences
